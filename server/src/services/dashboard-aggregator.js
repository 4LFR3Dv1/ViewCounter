const coveredStatuses = new Set(["connected", "syncing", "warning", "manual_mode"]);
const healthyStatuses = new Set(["connected", "syncing"]);
const staleStatuses = new Set(["warning", "expired", "disconnected"]);
...
function getSnapshotsByAccount(store) {
    return store.snapshots.reduce((map, snapshot) => {
...
function isCoveredAccount(account, latestSnapshot) {
...
function sumSnapshotForDay(snapshots, dayKey) {
    const match = snapshots.find((snapshot) => toDayKey(snapshot.capturedAt) === dayKey);
    return match?.totalViews ?? 0;
}
function buildGlobalChart(store, snapshotsByAccount) {
    const coveredAccounts = store.accounts.filter((account) => isCoveredAccount(account, getLatestSnapshot(snapshotsByAccount.get(account.id) ?? [])));
...
function buildPlatformSparkline(platformSlug, store, snapshotsByAccount, chartDates) {
    const accounts = store.accounts.filter((account) => account.platformSlug === platformSlug);
    return chartDates.map((capturedAt) => accounts.reduce((sum, account) => {
...
function resolvePlatformStatus(accounts, coveredCount) {
    if (coveredCount < accounts.length)
        return "warning";
    if (accounts.some((account) => account.status === "manual_mode"))
        return "manual_mode";
    if (accounts.some((account) => account.status === "syncing"))
        return "syncing";
    return "connected";
}
function buildPlatformCards(store, snapshotsByAccount, chartDates) {
    return store.platforms.map((platform) => {
        ...
        const coveredAccounts = accounts.filter((account) => isCoveredAccount(account, getLatestSnapshot(snapshotsByAccount.get(account.id) ?? [])));
...
function buildAccountCards(store, snapshotsByAccount, platformBySlug) {
    return store.accounts
        ...
        const platformAccounts = store.accounts.filter((item) => item.platformSlug === account.platformSlug && item.isActive);
        const coveredCount = platformAccounts.filter((item) => isCoveredAccount(item, getLatestSnapshot(snapshotsByAccount.get(item.id) ?? []))).length;
        return {
            id: account.id,
            ...
            coverageLabel: account.coverageNote ??
                `${coveredCount}/${platformAccounts.length} contas de ${platform.name} ativas`,
        };
    });
}
function buildWarnings(store, snapshotsByAccount, platformBySlug) {
    return store.accounts
        ...
        return {
            ...
            description: account.warningNote ??
                `Conta em estado ${account.status}. O layout permanece operacional usando a politica de cobertura parcial do backend.`,
            severity: account.status === "warning" || account.status === "expired" || account.status === "disconnected"
                ? "warning"
                : "info",
            latest,
        };
...
function buildEvents(store, platformBySlug) {
    return [...store.events]
...
function buildCases(store, platformBySlug) {
...
function buildHealth(store, snapshotsByAccount) {
    const activeAccounts = store.accounts.filter((account) => account.isActive);
    const covered = activeAccounts.filter((account) => isCoveredAccount(account, getLatestSnapshot(snapshotsByAccount.get(account.id) ?? []))).length;
    return {
...
function buildSummary(chart, store, coveredAccounts, snapshotsByAccount) {
    ...
    return {
        ...
        deltaPercentage: firstPoint && firstPoint.total > 0 && lastPoint
            ? calculateDelta(lastPoint.total, firstPoint.total)
            : 0,
        activeWindowLabel: "ultimos 7 snapshots",
    };
}
export function aggregateDashboard(store) {
    ...
    const coveredAccounts = store.accounts.filter((account) => isCoveredAccount(account, getLatestSnapshot(snapshotsByAccount.get(account.id) ?? [])));
    const chartDayKeys = [...new Set(coveredAccounts.flatMap((account) => (snapshotsByAccount.get(account.id) ?? []).slice(-7).map((snapshot) => toDayKey(snapshot.capturedAt))))]
        .sort((left, right) => left.localeCompare(right))
        .slice(-7);
    return {
