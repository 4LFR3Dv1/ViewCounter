...
function quoteEnum(value) {
    return quote(value);
}
function buildPlatformInserts(store) {
    return store.platforms.map((platform) => `INSERT INTO platforms (id, slug, name, color)
...
function buildAccountInserts(store) {
...
function buildSnapshotInserts(store) {
    return store.snapshots.map((snapshot) => `INSERT INTO metric_snapshots (
  id,
  account_id,
  captured_at,
  total_views,
...
function buildConnectionInserts(store) {
...
function buildEventInserts(store) {
...
function buildSyncJobInserts(store) {
...
function buildCaseInserts(store) {
